...
parser.add_argument('--video_file', type=str, default='data/Zoom_IN_OUT2.mp4', help='Path to target video')
...
def interactive_insertion():
    ...
    source_img = cv2.GaussianBlur(source_img, (3, 3), 1)
    ...
    # 保存结果
    cv2.imwrite(os.path.join(args.output_dir, "Zoom_IN_OUT2_final_insertion_result.png"), fused_img)
    cv2.imwrite(os.path.join(args.output_dir, "Zoom_IN_OUT2_source_img1.png"), source_img)
    cv2.imwrite(os.path.join(args.output_dir, "Zoom_IN_OUT2_mask_img1.png"), mask_img)
    with open(os.path.join(args.output_dir, "Zoom_IN_OUT2_insertion_params.json"), "w") as f:
...
def main():
    ...
    homography_json = os.path.join(args.output_dir, "Zoom_IN_OUT2_global_homography_results.json")
