...
def track_region_dense(video_file, region_points, resize_dim, visualize=False):
    ...
    tracked_regions = [region_pts.tolist()]
    ...
    while True:
        ...
        # 转 tensor
        tensor1 = torch.tensor(prev_frame).permute(2, 0, 1).unsqueeze(0).float() / 255.
        tensor2 = torch.tensor(frame2).permute(2, 0, 1).unsqueeze(0).float() / 255.
        tensor1 = tensor1.to(device)
        tensor2 = tensor2.to(device)

        with torch.no_grad():
            flow = RAFT(tensor1, tensor2)[-1]  # (1, 2, H, W)
        flow = flow.squeeze(0).permute(1, 2, 0).cpu().numpy()  # (H, W, 2)

        new_pts = []
        for (x, y) in region_pts:
            dx, dy = flow[int(round(y)), int(round(x))]
